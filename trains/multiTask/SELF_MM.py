...
class TA_GML():
    def __init__(self, args):
        ...
        self.args = args
        self.args.tasks = "M" + self.args.modal_ablation
        self.args.single_tasks = self.args.modal_ablation
        self.args.alpha = 0.5
        self.args.fake = args.fake
        ...
        self.confidence_predictor_map_1 = {
            'text': nn.Linear(args.expert_dim, args.expert_dim).to(args.device),
            'audio': nn.Linear(args.expert_dim, args.expert_dim).to(args.device),
            'vision': nn.Linear(args.expert_dim, args.expert_dim).to(args.device)
        }
        self.confidence_predictor_map_2 = {
            'text': nn.Linear(args.expert_dim, 1).to(args.device),
            'audio': nn.Linear(args.expert_dim, 1).to(args.device),
            'vision': nn.Linear(args.expert_dim, 1).to(args.device)
        }
    ...
    def feature_weighted_attention(self, feature, all_experts):
        # feature: [batch_size, expert_dim]
        # all_experts: [batch_size, expert_num, expert_dim]

        att_score = torch.matmul(all_experts, feature.unsqueeze(-1)).squeeze(-1)
        merge_feature = torch.matmul(att_score.unsqueeze(1), all_experts).squeeze(1)

        return merge_feature

    def confidence_predictor(self, feature, mode):
        # feature: [batch_size, expert_dim]
        # mode: 'text' or 'audio' or 'vision'
        pre1 = self.confidence_predictor_map_1[mode](feature)
        pre2 = self.confidence_predictor_map_2[mode](pre1)
        return pre2.squeeze(-1)

    def fake_label_generator(self, scores, labels):
        # scores: [batch_size]
        # labels: [batch_size]
        sigmoid = nn.Sigmoid()
        confi_socre = sigmoid(scores)
        fake_labels = 2 * labels * confi_socre - labels
        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    y_fake_true_batch = {'M': [], 'T': [], 'A': [], 'V': []}
                    Expert_f = outputs['Expert_f']
                    Expert_t = outputs['Expert_t']
                    Expert_a = outputs['Expert_a']
                    Expert_v = outputs['Expert_v']
                    all_experts = torch.stack([Expert_f, Expert_t, Expert_a, Expert_v], dim=1)
                    for m in self.args.single_tasks:
                        task_feature = self.feature_weighted_attention(outputs['Expert_' + m.lower()], all_experts)
                        confidence_score = self.confidence_predictor(task_feature, self.name_map[m])
                        fake_labels = self.fake_label_generator(confidence_score,
                                                                self.label_map[self.name_map['M']][indexes])
                        y_fake_true[m].append(fake_labels.cpu())
                        y_fake_true_batch[m].append(fake_labels.cpu())
                    # compute loss
                    loss = 0.0
                    for m in self.args.tasks:
                        if m != 'M' and self.args.fake:
                            loss += self.weighted_loss(outputs[m], self.label_map[self.name_map[m]][indexes], \
                                                       indexes=indexes, mode=self.name_map[m],
                                                       fake=torch.cat(y_fake_true_batch[m]).cuda())
                        else:
                            loss += self.weighted_loss(outputs[m], self.label_map[self.name_map[m]][indexes], \
                                                       indexes=indexes, mode=self.name_map[m])
                    # backward
            ...
            for m in self.args.tasks:
                if m != 'M' and self.args.fake:
                    pred, true, fake = torch.cat(y_pred[m]), torch.cat(y_true[m]), torch.cat(y_fake_true[m])
                    train_results = self.metrics(pred, self.args.alpha * true + (1 - self.args.alpha) * fake)
    ...
    def weighted_loss(self, y_pred, y_true, indexes=None, mode='fusion', fake=None):
        y_pred = y_pred.view(-1)
        y_true = y_true.view(-1)
        fake = fake.view(-1) if fake is not None else None
        if fake is not None and self.args.fake:
            # print("y_true is: ", y_true)
            # print("fake is: ", fake)
            y_true = self.args.alpha * y_true + (1 - self.args.alpha) * fake
